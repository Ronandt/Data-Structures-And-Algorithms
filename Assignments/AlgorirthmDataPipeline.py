...
class AlgorithmDataPipeline:
    ...
    def compare(arg1: str, arg2: str):
        for x, y in zip(arg1.lower().replace(" ", ""), arg2.lower().replace(" ", "")):
            if ord(x) > ord(y):
                return True
            elif ord(x) < ord(y):
                return False
    ...
    def updateRecord(arg1: object):
        ...
        while 1:
            raw_package = input(
                "Change package name (input nothing to skip): ")
            if raw_package != "":
                if not raw_package.isalpha():
        ...
        while 1:
            raw_customer = input(
                "Change customer name (input nothing to skip): ")
            if raw_customer != "":
                if not raw_package.isalpha():
                    print("Must contain only letters")
                    continue
        ...
        while 1:
            try:
                raw_pax = input("Change number of pax (input nothing to skip): ")
                if raw_pax != "":
                    pax = int(raw_pax)
        ...
        while 1:
            try: 
                raw_package_cost = input("Change package cost per pax(input nothing to skip): ")
                if raw_package_cost !="":
